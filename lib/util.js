...
var APPC_DIR = path.join(home(), '.appcelerator', 'install');
...
function parseTiConfigFile (value) {
  var tiConfigFile = path.join(home(), '.titanium', 'config.json');
  var config = JSON.parse(fs.readFileSync(tiConfigFile));
  if(value) {
    return config.hasOwnProperty(value) ?  config[value] : new Error('Value does not exist in ti config');
  } else {
    return config;
  }
}
...
function getInstalledCores (cb) {
  var cores = [];
  var files = fs.readdirSync(APPC_DIR);
  for(var file in files) {
    var loc = path.join(APPC_DIR, files[file]);
    if(fs.lstatSync(loc).isDirectory()) {
      cores.push(files[file]);
    }
  }
  cb(null, cores);
}
...
function removeSDK (version, cb) {
  exec('appc ti sdk uninstall --force ' + version, function (err, stdout, stderr) {
    if(err) {cb(err, version);}
    cb(null, version);
  });
}

function removeCore (version, cb) {
  exec('appc remove ' + version + ' --force', function (err, stdout, stderr) {
    if(err) {cb(err, version);}
    cb(null, version);
  });
}
  ...
  switch (os) {
    ...
    // ],
      for(var i in devices) {
        deviceString += devices[i].name + ' SDK v' + devices[i].wpsdk;
      }
...
exports.getInstalledCores = getInstalledCores;
...
exports.removeCore = removeCore;
exports.removeSDK = removeSDK;
exports.configDir = configDir;
exports.configFile = configFile;
exports.parseTiConfigFile = parseTiConfigFile;
